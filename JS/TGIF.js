...
function senateDataInTable(data) {
  ...
  for (i = 0; i < data.length; i++) {
      if (data[i].party == "R") {
        resultnext = `<tr><td><a href="${data[i].url}"> ${data[i].first_name}</a></td><td> ${data[i].middle_name || ""} </td><td> ${data[i].last_name} </td><td> Republican </td><td> ${data[i].state} </td><td id=pctPartyCol> ${data[i].votes_with_party_pct} %</td></tr>`;
      } else if (data[i].party == "D") {
        resultnext = `<tr><td><a href="${data[i].url}"> ${data[i].first_name}</a></td><td> ${data[i].middle_name || ""} </td><td> ${data[i].last_name} </td><td> Democrat </td><td> ${data[i].state} </td><td id=pctPartyCol> ${data[i].votes_with_party_pct} %</td></tr>`;
      } else if (data[i].party == "I") {
        resultnext = `<tr><td><a href="${data[i].url}"> ${data[i].first_name}</a></td><td> ${data[i].middle_name || ""} </td><td> ${data[i].last_name} </td><td> Independent </td><td> ${data[i].state} </td><td id=pctPartyCol> ${data[i].votes_with_party_pct} %</td></tr>`;
      }
    result += resultnext;
    }
  return result;
